...
impl Contract {
    ...
    pub async fn restore_data(&mut self, caller: ActorId) -> RutzoEvent {
        ...
        for (user, tokens_metadata) in data_to_restore.into_iter() {
            ...
            // [TODO]: check contract restore, to restore sessions accounts

            if user != exec::program_id() {
                self.register_user(user, ActorId::default());
            }
    ...
    pub fn register_user(&mut self, user_id: UserId, session_address: ActorId) -> RutzoEvent {
        if self.is_register(&user_id) {
